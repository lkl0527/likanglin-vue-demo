    ...
    methods: {
      ...
      deleteUser(scope) {
        ...
        this.$confirm('此操作将永久删除该用户, 是否继续?', '提示', {
          ...
          this.http(this, 'delete', this.api.user.deleteUser + row.userId).then((data) => {
            this.tableData.splice(index, 1);//删除table中对应的哪一行数据
            this.loadUsers();
            this.$message({
              type: 'success',
              message: '删除成功!'
       ...
       */
      loadOrgas() {
        this.http(this, 'get', this.api.orga.getOrga).then((data) => {
          if (data != null || data != undefined || data != '') {
            let map = {};
            data.forEach(function (orga) {
